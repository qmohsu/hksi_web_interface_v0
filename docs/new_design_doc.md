...
Confirm start line geometry (two buoys/anchors), calibration status

Confirm all athletes connected and time sync healthy

Live start monitoring

Watch ranking board + map

Receive alerts (risk/OCS/cross)

Tap an athlete to focus (camera follow, highlight track, show detailed metrics)

Post-session replay

Select session → replay

Scrub timeline, vary playback speed

Export data (CSV/JSON) and generate quick "session summary"

Operational troubleshooting

Device health view (battery, link quality, packet loss, time sync offsets)

Identify missing athlete tag, failing anchor, degraded link
...
**What HKSI_Pos does NOT provide** (must be handled by relay or UI backend):
...
Fields added by relay (not from HKSI_Pos): `athlete_id`, `name`, `team`, `sog_kn`, `cog_deg`, `data_age_ms`. Fields passed through from HKSI_Pos: `device_id`, `lat`, `lon`, `alt_m`, `source_mask`, `device_ts_ms` (converted from µs).
...
Fields added by relay: `athlete_id`, `name`, `status` (classified). Fields renamed from HKSI_Pos: `d_perp_signed_m` → `dist_to_line_m`, `time_to_line_s` → `eta_to_line_s`. Timestamps converted from µs to ms.

**Message type: `start_line_definition`** (on session start + when anchors move significantly)
...
9. Demo / Simulation mode (to persuade stakeholders & unblock integration)
